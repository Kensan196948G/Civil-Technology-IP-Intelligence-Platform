...
計画的な再起動や一瞬のネットワーク断でも通知が出て**警報疲れ**を招き、本当の障害が埋もれる。

| 連続失敗回数 | 挙動 |
|---|---|
| 1〜2 | 警告ログのみ。unit は**成功扱い**（exit 0） |
| **3以上** | unit が**失敗**（exit 1）→ `OnFailure` により `~/.local/state/ctiip/failed-units/` へマーカー記録 |
| 成功時 | カウンタを 0 にリセット |

- カウンタ: `~/.local/state/ctiip-healthcheck/consecutive-failures`
- しきい値の変更: `CTIIP_HEALTHCHECK_FAILURE_THRESHOLD`（既定 3）

> **この仕組みが必要だと分かった経緯**: 2026-09-10 に本スクリプト自身が、MVP の
> 計画的な再起動中に単発 failure を記録し unit を failed にした（マーカーが残った）。
> 監視が自分の弱点を検出した形であり、その記録を根拠にしきい値を実装した。
