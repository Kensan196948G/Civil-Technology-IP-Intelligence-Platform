...
| **CI の `Cloudflare Pages build（next-on-pages）verification` ステップが恒常的に失敗する（2026-08-29〜）** | DBドライバ（`postgres.js`。Node.js組み込みモジュール依存）を使う現行のNode.js自社ホスト構成（ADR-0007）と、`next-on-pages`が要求するEdge Runtime（全ルートに `export const runtime='edge'` が必要）が本質的に非両立。全249ルートをEdge化するとDB接続自体が壊れるため対応していない。**既知・未決事項（拡張計画書 D-5）**。`.github/workflows/deploy-production.yml` もこのCloudflare Pages経路のままADR-0007後の実態と不整合であることが判明したが、実際の自社ホストデプロイ手順の詳細情報が無く再設計は保留（2026-09-06、ユーザー承認済み）。**暫定対応として `ci.yml` の当該ステップを `continue-on-error: true` にし、`quality`ジョブの合否から切り離した**（lint/typecheck/build/audit/secret-scanの成否のみでPRの品質を判断する） | [ADR-0007](docs/20-architecture/adr/ADR-0007-local-postgresql.md) |
...
> レスポンス本文に機密は一切含まれない。厳密な 404 応答が必要になった場合は、middleware での事前判定（DB照会付き）を検討する。
