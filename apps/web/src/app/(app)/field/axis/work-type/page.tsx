...
export default async function FieldAxisWorkTypePage() {
  const rows = await loadFieldAxisRows(['工種適合性', '工種']);

  return (
    <ListView
      title="現場適用性評価 — 工種適合性"
      moduleCode="S-19 / FIELD APPLICATION AXIS"
      description="現場適用性スコアのうち「工種適合性」軸の評価内訳です。各行から現場適用性評価の詳細（全軸の内訳）を確認できます。"
      rows={rows}
      emptyMessage="「工種適合性」軸の評価データはまだありません。現場・課題ページから困りごとを登録すると、AIが候補技術と軸別スコアを提案します。"
