...
export default async function TechIctPage() {
  const db = getDb(getDatabaseUrl());
  const rows = await db.select().from(s.technologies)
    .where(or(ilike(s.technologies.name, '%ICT%'), ilike(s.technologies.summary, '%ICT%')))
  ...
  return (
    <ListView
      ...
      description="技術台帳から名称・概要に「ICT」を含む技術を抽出した一覧です。"
      rows={rows}
      emptyMessage="ICT施工に関連する技術データがまだありません。"
      fields={[
