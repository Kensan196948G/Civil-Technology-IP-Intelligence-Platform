...
export default async function TechBimCimPage() {
  const db = getDb(getDatabaseUrl());
  const rows = await db.select().from(s.technologies)
    .where(or(
      ilike(s.technologies.name, '%BIM%'), ilike(s.technologies.summary, '%BIM%'),
      ilike(s.technologies.name, '%CIM%'), ilike(s.technologies.summary, '%CIM%')
    ))
  ...
  return (
    <ListView
      ...
      description="技術台帳から名称・概要に「BIM」「CIM」を含む技術を抽出した一覧です。"
      rows={rows}
      emptyMessage="BIM / CIMに関連する技術データがまだありません。"
      fields={[
