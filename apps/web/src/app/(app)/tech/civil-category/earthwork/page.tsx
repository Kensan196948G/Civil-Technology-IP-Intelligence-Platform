...
export default async function CivilCategoryEarthworkPage() {
  const rows = await loadCivilCategoryRows(['earthwork'], '土工');

  return (
    <ListView
      title="土木技術・現場適用 — 土工"
      moduleCode="S-06 / CIVIL TECH CATEGORY"
      description="工種区分「土工」に該当する他社特許・自社技術・NETIS登録技術を横断表示します。"
      rows={rows}
      emptyMessage="工種区分「土工」に該当する技術・特許はまだありません。"
