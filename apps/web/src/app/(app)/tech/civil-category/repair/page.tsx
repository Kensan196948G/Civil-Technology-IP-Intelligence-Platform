...
export default async function CivilCategoryRepairPage() {
  const rows = await loadCivilCategoryRows(['repair', 'reinforcement'], '補修');

  return (
    <ListView
      title="土木技術・現場適用 — 補修・補強"
      moduleCode="S-06 / CIVIL TECH CATEGORY"
      description="工種区分「補修・補強」に該当する他社特許・自社技術・NETIS登録技術を横断表示します。"
      rows={rows}
      emptyMessage="工種区分「補修・補強」に該当する技術・特許はまだありません。"
