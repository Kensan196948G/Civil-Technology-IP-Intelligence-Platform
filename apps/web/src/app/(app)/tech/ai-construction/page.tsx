...
export default async function TechAiConstructionPage() {
  const db = getDb(getDatabaseUrl());
  const rows = await db.select().from(s.technologies)
    .where(or(
      ilike(s.technologies.name, '%AI%'), ilike(s.technologies.summary, '%AI%'),
      ilike(s.technologies.name, '%自動施工%'), ilike(s.technologies.summary, '%自動施工%'),
      ilike(s.technologies.name, '%自動化%'), ilike(s.technologies.summary, '%自動化%')
    ))
  ...
  return (
    <ListView
      ...
      description="技術台帳から名称・概要に「AI」「自動施工」「自動化」を含む技術を抽出した一覧です。"
      rows={rows}
      emptyMessage="AI・自動施工に関連する技術データがまだありません。"
      fields={[
